...
| <a name="input_security_group_names"></a> [security\_group\_names](#input\_security\_group\_names) | The name to assign to the created security group. Must be unique within the VPC.<br>If not provided, will be derived from the `null-label.context` passed in.<br>If `create_before_destroy` is true, will be used as a name prefix. | `list(string)` | `[]` | no |
...
| <a name="input_security_group_rules"></a> [security\_group\_rules](#input\_security\_group\_rules) | Map of security group rules | <pre>map(object({<br>    cidr_blocks = list(string)<br>    protocol    = string<br>    type        = string<br>    description = optional(string)<br>    from_port   = optional(number)<br>    to_port     = optional(number)<br>  }))</pre> | `null` | no |
| <a name="input_security_group_tags"></a> [security\_group\_tags](#input\_security\_group\_tags) | A map of tags to assign to the created security group. | `map(string)` | `{}` | no |
...
| <a name="input_vpc_name"></a> [vpc\_name](#input\_vpc\_name) | The name of the VPC where the Security Group will be created. | `string` | `null` | no |
